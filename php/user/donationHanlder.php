...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (
        empty($f_name) || empty($l_name) || empty($username) || empty($email) || empty($tel) || empty($gender) ||
        empty($birth_date) || empty($blood_group) || empty($city) || empty($postal_code) ||
        trim($f_name) == '' || trim($l_name) == '' || trim($username) == '' ||
        trim($email) == '' || trim($tel) == '' || trim($gender) == '' || trim($birth_date) == '' ||
        trim($blood_group) == '' || trim($city) == '' || trim($city) == ''
    ) {
    ...
    if(!empty($user)){
        $_SESSION['Existerror'] = "User already exists! Try another username or login";
        header("Location: " . $_SERVER['HTTP_REFERER']);
        exit();
    }

    $query = "INSERT INTO users (first_name, last_name, username, email, phone_number, gender, blood_group, birth_date) 
    VALUES (:f_name, :l_name, :username, :email, :tel, :gender, :blood_group, :birth_date)";
